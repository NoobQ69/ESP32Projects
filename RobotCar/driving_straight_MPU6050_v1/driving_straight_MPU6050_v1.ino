...
const int leftSpeedPin = 9; //means pin 9 on the Arduino controls the speed of left motor
...
Servo servoMotor1;
Servo servoMotor2;
int servo1Pin = 32; // Example, need to redefine
int servo2Pin = 33;
int minUs = 500;
int maxUs = 2500;
...
void loop()
{
  ...
  if(Serial.available() || SerialBT.available() > 0)
  {
    char getSerialValue;
    if (Serial.available())
    {
    ...
    else if (getSerialValue == 'i') //print information. When car is stationary, GyroX should approx. = 0.
    { 
      Serial.print("angle: ");
...
void driving () //called by void loop(), which isDriving = true
{
  ...
  if (deltaAngle != 0)
  {
    ...
    analogWrite(rightSpeedPin, rightSpeedVal);
    analogWrite(leftSpeedPin, leftSpeedPinVal);
  }
}
...
void forward()
{ //drives the car forward, assuming leftSpeedPinVal and rightSpeedVal are set high enough
...
void left()
{ //rotates the car left, assuming speed leftSpeedPinVal and rightSpeedVal are set high enough
  digitalWrite(right1, LOW);
