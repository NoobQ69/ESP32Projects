...
// ----- FreeRTOS varriables definition -----
...
// ------ NTP time varriables definition -------
...
// ----- WiFi Manager varriables definition -----
WiFiManager wm;
String Web_App_URL = "";
// ----------------------------------------------

// ----- RTC varriables definition -----
...
// ----- R557 finger print varriables definition -----
uint8_t Id;
Adafruit_Fingerprint finger = Adafruit_Fingerprint(&Serial2);
// -------------------------------------

// ----- system struct varriables definition -----
...
/// FOR R557 FINGERPRINT SENSOR

// Register finger
uint8_t getFingerprintEnroll() {
  unsigned int timeCountTo = 0, timeCountToMinute = 0, blinkWatch = 0;
  int p = -1;
#ifdef DEBUG_ON
...
#ifdef DEBUG_ON
        Serial.println(".");
        Serial.print(timeCountTo++);
#endif
        vTaskDelay(10/portTICK_PERIOD_MS);
        timeCountTo++;
        if (timeCountTo > 6000-1)
        {
          timeCountToMinute++;
          if (timeCountToMinute > 0)
          {
            wm.setAbort(true);
  ...
  BuzzerMakesSound(BUZZER_SUCCESS_READ_WRITE);
  uint8_t pTemp = 0; 
  while (pTemp != FINGERPRINT_NOFINGER) {
    pTemp = finger.getImage();
    // delay(100);
    vTaskDelay(100 / portTICK_PERIOD_MS);
  }
...
void secondLoopTask(void *parameter) {
  while (1) {
    if (wm.getIDAcknowledge()) {
      system1.fingerPrintMode = R557_REGISTER_MODE;

      Id = wm.getIDString().toInt();  // ConvertToIDByteArray(dataBlock, wm.getIDString());
    ...
    switch (system1.fingerPrintMode) {
      case R557_READ_MODE:
        {
          int getFingerprintResult = getFingerprintID();
          if (getFingerprintResult == FINGERPRINT_OK) {
            String getCurrentUserAttendance = "&uid=";
            getCurrentUserAttendance += String(Id);  // byteArrayToString(buffer, 4);
          ...
          } else if (getFingerprintResult == FINGERPRINT_NOFINGER) {
            // do nothing
          } else {
            BuzzerMakesSound(BUZZER_ERROR_READ_WRITE);
          }
      ...
      case R557_REGISTER_MODE:
        {
          int getEnrollResult = getFingerprintEnroll();

          if (getEnrollResult == FINGERPRINT_OK) {
            system1.fingerPrintMode = R557_READ_MODE;

            BuzzerMakesSound(BUZZER_SUCCESS_READ_WRITE);
            // BlinkLED(LED_CARD, 1, 200);
          }
          else if (getEnrollResult == FINGERPRINT_NOFINGER)
          {
            system1.fingerPrintMode = R557_READ_MODE;
            SetLED(LED_CARD, LED_OFF);
          } 
            ...
            SetLED(LED_CARD, LED_OFF);
            system1.fingerPrintMode = RC557_ERROR_MODE;
            BuzzerMakesSound(BUZZER_ERROR_READ_WRITE);
          }
          break;
        }
      case RC557_ERROR_MODE:
        {
          // handleError
          system1.fingerPrintMode = R557_READ_MODE;
          break;
