...
unsigned char Code7SegSource[] = {0x3F, 0x06, 0x5B, 0x4F, 0x66, 0x6D, 0x7D, 0x07, 0x7F, 0x6F, 0x77/*A*/, 0x7C/*B*/, 0x39/*C*/, 0x5E/*D*/, 0x79/*E*/, 0x71/*F*/, 0x00/* */, 0x08};
...
unsigned char SteamMode = 1; // 0: Mode Stop, 1: Mode Start
unsigned int Temperature = 100, RoomTemperature = 0x888, ChangeToPrintTemp = 0;
unsigned char KeyBuffer[3] = {0, 0, 0}; // 0: Mode Stop, 1: Mode Start

unsigned int PreviousTime = 0, PreviousSecond = 0, CurrentTime, TimeInterval = 10, UpToOneTime = 5000; // 30ms
...
uint8_t ReadButtons(void){
  ...
  if (RepeatTime > 50)
  {
	...
	if (KeyBuffer[0] != 0 && (KeyBuffer[0] == KeyBuffer[1])&&(KeyBuffer[2] == 0))
	{
    RepeatTime = 0;
    EnableKeyRepeat = 0;
		result = KeyBuffer[0];
    if (result == 1)
    {
      BuzzerTime = SETUP_SOUND;
    }
    else if (result == 3 && MemorSettingMode == 0 && SteamMode == 0)
    {
      BuzzerTime = TRANSMIT_SOUND;
    }
    else
    {
      BuzzerTime = PUSHBUTTON_SOUND;
    }
	}
...
{
  PrintLED7(RoomTemperature%1000/100, TEMP1);
  PrintLED7(RoomTemperature%100/10, TEMP2);
  PrintLED7(RoomTemperature%10, TEMP3);
}
...
void loop() {
  ...
  if ((millis() % 65535) - PreviousSecond > UpToOneTime)
  {
